...
class UserMenuController:
    ...
    def update_user(self):
        ...
        try:
            ...
            if updated_data['email'] != selected_user.email:
                existing_user = db.query(User).filter_by(email=updated_data['email']).first()
    ...
    def delete_user(self):
        ...
        try:
            ...
            clients_count = db.query(Client).filter_by(commercial_id=selected_user.id).count()
            contracts_count = db.query(Contract).filter_by(commercial_id=selected_user.id).count()
            events_count = db.query(Event).filter_by(support_id=selected_user.id).count()

            if clients_count > 0 or contracts_count > 0 or events_count > 0:
                db.close()
                show_error(
                    f"Impossible de supprimer cet utilisateur. Il est associé à:\n"
                    f"- {clients_count} client(s)\n"
                    f"- {contracts_count} contrat(s)\n"
                    f"- {events_count} événement(s)\n"
